 ...
 */
export function dateStringToYymmdd(dateString: DateString): `${number}` {
  return (dateStringToInteger(dateString) as number)
    .toString()
    .slice(-6) as `${number}`
}
 ...
 */
export function yymmddToDateString(yymmdd: string): DateString {
  resetCurrentDate()
  ...
  return dateIntegerToString(year * 10_000 + month * 100 + day) as DateString
}
 ...
 */
export function yyToYyyy(yy: number): number {
  resetCurrentDate()
